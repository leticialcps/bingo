...
if menu == "Fazer Aposta":
    ...
    # Verifica se é dia 27/12 às 15h ou depois (horário de Brasília UTC-3)
    from datetime import timezone, timedelta
    brasilia_tz = timezone(timedelta(hours=-3))
    agora_brasilia = datetime.now(brasilia_tz)
    
    apostas_bloqueadas = (agora_brasilia.day == 27 and 
                         agora_brasilia.month == 12 and 
                         agora_brasilia.hour >= 15)
    ...
    if user_id:
        ...
        # Verifica se é dia 27/12 a partir das 15h (horário de Brasília UTC-3)
        from datetime import timezone, timedelta
        brasilia_tz = timezone(timedelta(hours=-3))
        agora_brasilia = datetime.now(brasilia_tz)
        
        # Disponível o dia todo no dia 27/12, mas OBRIGATÓRIO a partir das 15h
        dia_27 = (agora_brasilia.day == 27 and agora_brasilia.month == 12)
        apos_15h = (agora_brasilia.hour >= 15)
        
        vinculo_obrigatorio = dia_27 and apos_15h
        vinculo_disponivel = dia_27
        
        # Se for antes do dia 27/12, pede senha para vincular
        ...
        # Se for após 15h do dia 27/12, apenas mostra backup e não permite apostas
...
# -----------------------------------------------
    ...
    # Verifica se é dia 27/12
    hoje = datetime.now()
    dia_revelacao = (hoje.day == 27 and hoje.month == 12)
    
    # Solicita senha apenas se NÃO for dia 27/12
    ...
    if not dia_revelacao:
        ...
        st.info("🎉 Vamos para as revelações.")
...
# -----------------------------------------------
    ...
    if len(revelacoes) == 0 or all(v == "Ainda não revelado" for v in revelacoes.values()):
        ...
        # Verifica se hoje é 27/12 para revelar nomes reais
        hoje = datetime.now()
        dia_revelacao = (hoje.day == 27 and hoje.month == 12)
